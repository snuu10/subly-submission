...
import type { LifecycleAlert, LifecycleStatus } from '@/types';
...
export async function listLifecycleAlerts(): Promise<LifecycleAlert[]> {
  ...
  return alerts.filter((item): item is Record<string, unknown> => Boolean(item && typeof item === 'object')).map((rec) => ({
    subscription_id: String(rec.subscription_id ?? ''),
    name: String(rec.name ?? ''),
    kind: rec.kind === 'end_confirm' ? 'end_confirm' as const : 'ending_soon' as const,
